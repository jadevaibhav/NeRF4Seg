...
def visualize_plot(model, img, result, out_file):
    img = model.show_result(
        img, result, palette=model.PALETTE, show=False, opacity=0.5)
...
def main():
    ...
    for image in input_image_ls:
        ...
        dictionary = {}
        flat = result[0].reshape(result[0].shape[0]*result[0].shape[1],)
        for item in flat:
            if item in dictionary.keys():
                dictionary[item] += 1
            else:
                dictionary[item] = 1
        mapper = ['aeroplane', 'bag', 'bed', 'bedclothes', 'bench', 'bicycle',
