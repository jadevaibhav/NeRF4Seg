...
class FlexibleNeRFModelSeg(torch.nn.Module):
    def __init__(
        self,
        num_layers=4,
        hidden_size=128,
...
class FlexibleNeRFModelSegClass(torch.nn.Module):
    def __init__(
        self,
        num_layers=4,
        hidden_size=128,
        ...
        super(FlexibleNeRFModelSegClass, self).__init__()
        ...
        if self.use_viewdirs:
            ...
            self.fc_alpha = torch.nn.Linear(hidden_size, 1)
            self.fc_class = torch.nn.Linear(hidden_size, 59)
            self.fc_rgb = torch.nn.Linear(hidden_size // 2, 3)
            self.fc_feat = torch.nn.Linear(hidden_size, hidden_size)
        else:
            self.fc_out = torch.nn.Linear(hidden_size, 4+59)
    ...
    def forward(self, x):
        ...
        if self.use_viewdirs:
            feat = self.relu(self.fc_feat(x))
            alpha = self.fc_alpha(x)
            class_p = self.fc_class(x)
            x = torch.cat((feat, view), dim=-1)
            for l in self.layers_dir:
                x = self.relu(l(x))
            rgb = self.fc_rgb(x)
            return torch.cat([rgb, alpha, class_p], dim=-1)
        else:
            return self.fc_out(x)